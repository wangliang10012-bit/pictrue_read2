...
class Page3Manager:
    ...
    def create_text_image_for_page3(self, base_filename, text_positions):
        ...
        try:
            ...
            logger.log_error(f"创建第三页文字图片失败: {base_filename}", e)
            return None, None
...
class ThirdPageScreen(Screen):
    ...
    def on_screen_touch(self, instance, touch):
        ...
        logger.log(f"第三页点击图片位置：({touch_x:.1f}, {touch_y:.1f}), 图片尺寸：{img_width}x{img_height}")

        if 0 <= touch_x <= 60 and 780 <= touch_y <= 820:
            logger.log("点击了返回按钮，跳回第二页")
